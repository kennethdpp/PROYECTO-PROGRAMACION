...
def valfloat(*args):
    ...
    if len(args)==2:
        ...
        if numero<min(lista) or numero>max(lista):
            raise ValueError('El primer argumento no está en el intervalo de la lista')
        return True
...
def valint(*args):
    ...
    if len(args)==2:
        ...
        if not isinstance(lista,list):
            raise TypeError('El segundo argumento debe ser una lista de enteros')
        if len(lista)!=2:
...
def valcomplex(*args):
    ...
    if len(args)==2:
        ...
        negativo=-modulo
        if modulo<0:
            if negativo<min(lista) or negativo>max(lista):
                raise ValueError('El primer argumento no está en el intervalo de la lista')
            else: return True
        if modulo>=0:
            if modulo<min(lista) or modulo>max(lista):
                raise ValueError('El primer argumento no está en el intervalo de la lista')
            else:
                return True
        return True